...
public class If_for {
	...
	public static void main(String[] args) {
		...
		for(f=1;f<11;f++) {
         ...
         if(ok==false) {  //만약에 반복문이 끝났는데도 반복문 안에 조건 사항이
        	              //없을경우 출력되는 코드입니다.
        	 System.out.println("확인이 되지 않은 숫자입니다.");
         }
		
         /* 총 20번의 값을 반복합니다. 그 중 홀수값, 짝수값을 출력하도록 합니다.*/
         
         int ff;
         String odd=""; 
         for(ff=1;ff<21;ff++) {
        	 if(ff % 2 != 1) {    // 홀수는 odd"1", 짝수는 even"0"
        		 System.out.print(ff+" ");
        	 }
        	 else {    //만약에 홀수 값일 경우 odd 변수에 지속적으로 값을 추가하게됩니다.
        		 odd += ff + " ";   //추가 할 경우 "  " 빈공간을 활용하여 적용
        	 }
